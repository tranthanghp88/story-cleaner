...
// 1. Load configuration from cache
...
// 2. Load chapter 1 clean text from cache
const chapter = cache.chapters[0];
const rawText = chapter.cleaned || chapter.raw;
...
async function callGemini(prompt) {
  ...
  const model = cache.apiSettings.model || 'gemini-2.5-flash-lite';
  
  console.log(`\nCalling Gemini model ${model} using key ${key.substring(0, 10)}...`);
  ...
  if (!response.ok) {
    ...
    throw new Error(`Gemini API error! status: ${response.status}, response: ${errText}`);
  }
...
async function main() {
  // Output prompt sent for sentence 19 (for question 4)
  console.log("=== EXCELLET PROMPT FOR ONE FLAGGED ITEM (Sentence 19) ===");
  const item19 = flaggedItems.find(x => x.sentenceId === 19);
  console.log("Item 19 context & original:");
  console.log(JSON.stringify(item19, null, 2));
  
  try {
    const geminiResponseText = await callGemini(selectivePrompt);
    console.log("\n=== GEMINI RESPONSE ===");
    console.log(geminiResponseText);
    
    const editedList = JSON.parse(geminiResponseText);
    ...
    editedList.forEach(item => {
      ...
      if (seg) {
        ...
        console.log(`\nWarning: Sentence ID ${sId} not found in local segments!`);
      }
    });
