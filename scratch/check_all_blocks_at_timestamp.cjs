...
const targetTime = '2026-06-22 01:11:09';
const timeBlocks = blocks.filter(b => b.time === targetTime);
console.log(`\nFound ${timeBlocks.length} blocks at ${targetTime}:`);

timeBlocks.forEach((tb, i) => {
  const firstLine = tb.content.split('\n')[0].trim();
  console.log(`[${i}] Header: "${firstLine}" Length: ${tb.content.length}`);
});
